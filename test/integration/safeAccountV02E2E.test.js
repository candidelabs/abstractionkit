...
beforeAll(async () => {
	...
	if (!bundlerAvailable) {
		console.warn(
			"Voltaire bundler not reachable — skipping Safe V0_2_0 e2e tests",
		);
	}
});
...
describe("Safe account e2e — V0_2_0", () => {
	let smartAccount;
	let accountAddress;

	beforeAll(async () => {
		accountAddress = ak.SafeAccountV0_2_0.createAccountAddress([
			owner.address,
		]);
	...
	test("create, sign, and send UserOp to deploy account", async () => {
		...
		smartAccount = ak.SafeAccountV0_2_0.initializeNewAccount([
			owner.address,
		]);
	...
	test("add owner", async () => {
		...
		smartAccount = new ak.SafeAccountV0_2_0(accountAddress);
