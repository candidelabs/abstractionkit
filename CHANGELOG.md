...
- **`UserOperationReceipt.logs` and `UserOperationReceiptResult.logs` are now structured `Log[]`** instead of a JSON-encoded string. Callers that previously did `JSON.parse(receipt.logs)` should drop the parse and read the array directly:
  ...
  ```
- **Removed the IIFE / UMD (`unpkg`) browser build.** `dist/index.iife.js` and the `unpkg` field in `package.json` were removed. The `<script>` / CDN build was unused and shipped broken (it externalized `@noble/*` as undefined globals, so loading it threw `ReferenceError`). Install via npm and use a bundler; the ESM (`dist/index.mjs`) and CJS (`dist/index.cjs`) entries are unchanged.

### Bug Fixes

- **Browser and React Native compatibility.** `generateOnChainIdentifier` used `Buffer` and ABI `string` decoding used `TextDecoder`, both undefined in browsers without a polyfill and in React Native / Hermes, so those paths threw `ReferenceError`. They now use pure-JS UTF-8 helpers (`toUtf8Bytes` and the new `fromUtf8Bytes`), so the SDK runs in browsers (Vite, webpack, esbuild) and React Native out of the box. `fromUtf8Bytes` rejects malformed UTF-8 (overlong, surrogate-range, out-of-range, and truncated sequences) by emitting U+FFFD to match `TextDecoder`, instead of coercing the bytes into other characters. The Calibur executor-calldata decode now hex-encodes the inner `bytes` payload instead of UTF-8-decoding it, which had corrupted the selector and arguments.
