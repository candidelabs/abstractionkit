...
// `tokenQuote` carries the exchange rate, the max token cost, the paymaster the
// approval was granted to (verified to match the returned op's paymaster), and
// the allowance amount that approval sets (`approveAmount`).
