...
export function verifyMptProof(params: {
	...
	for (let i = 0; i < proof.length; i++) {
		...
		const actualHash = nodeRlp.length >= 32 ? hexToBytes(keccak256(nodeRlp)) : nodeRlp;
...
export function verifyAccountProof(params: {
	...
	try {
		...
		throw new AccountProofInvalidError(address, stateRoot, 0n, (e as Error).message);
	}
...
export function verifyStorageProof(params: {
	...
	try {
		...
		throw new StorageProofInvalidError(storageKey, storageHash, (e as Error).message, address);
	}
