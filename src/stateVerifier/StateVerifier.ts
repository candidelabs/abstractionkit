...
import { CodeHashMismatchError } from "./errors";
import {
	EMPTY_CODE_HASH,
	EMPTY_STORAGE_HASH,
	verifyAccountProof,
...
function normalizeSlot(slot: string | bigint | number): string {
	...
	if (asBig < 0n || asBig > MAX_UINT256) {
		throw new RangeError(`Invalid slot: ${slot} is outside the uint256 range [0, 2^256 - 1]`);
	}
...
export class StateVerifier {
	...
	constructor(params: {
		...
		this.consensusRpcs = Array.from(new Set([params.primaryRpc, ...params.verificationRpcs]));

		const quorumDefault = Math.floor(this.consensusRpcs.length / 2) + 1;
		const quorum = params.quorumThreshold ?? quorumDefault;
		if (!Number.isInteger(quorum) || quorum < 1 || quorum > this.consensusRpcs.length) {
	...
	async getVerifiedAccountState(params: {
		...
		const header =
			params.header ??
			(await this.getConsensusBlockHeader({
				blockNumber: params.blockNumber ?? "latest",
			}));
		...
		const returnedSlotSet = new Set(proof.storageProof.map((sp) => normalizeSlot(sp.key)));
	...
	async getVerifiedCode(params: {
		...
		const actualCodeHash = code === "0x" ? EMPTY_CODE_HASH : keccak256(code).toLowerCase();
